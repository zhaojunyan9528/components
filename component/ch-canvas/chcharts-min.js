...
function E(that, context, width, height) {
  ...
  var step = 1;
	//动画速度
  var MaxStep = that.options.animation ? 90 : 1;
  var animation = function() {
    if (step <= MaxStep) {
      for (var layer = 5; layer > 0; layer--) {
        ...
        context.setGlobalAlpha(1);
				//所绘线条颜色
				context.setStrokeStyle(that.options.lineColor);
				//线条区域填充颜色
        if (layer % 2 != 0) {
      ...
      for (var i = 0; i < length; i++) {
        ...
        // else context.setTextBaseline("bottom");
				...
				context.setFontSize(12);
				context.setFillStyle("#666");
				context.fillText(that.options.xLabel[i], curX, curY);
				if (that.options.xLabelData){
					context.setTextAlign("center");
					context.setFillStyle("#333");
					context.setFontSize(16);
					context.fillText(that.options.xLabelData[i], curX, curY - 20);
				}
        currentAngle += angle
      }
      context.closePath();
    ...
    if (step <= MaxStep) {
      ...
      that.options.data.forEach(function(val, idx) {
        ...
        context.setFillStyle("#ffffff");
				//radius * 0.9 控制弧径宽度
        context.arc(center.x, center.y,radius * 0.85, start, start + val.value / MaxStep * step / total * 2 * Math.PI, false);
        context.lineTo(center.x, center.y);
        context.fill();
        context.closePath();
        ...
        if (val.value > 0 && that.options.showLegend) {
          ...
          context.closePath();
			...
			context.arc(center.x - 25, center.y , 10, 0.5 * Math.PI, 1.5 * Math.PI);
			context.lineTo(center.x + 25, center.y -10);
			context.arc(center.x + 25, center.y , 10, 1.5 * Math.PI, 0.5 * Math.PI);
			context.lineTo(center.x - 25, center.y + 10);
			context.fill();
			context.setFontSize(14);
			context.setFillStyle("#E60012");
			context.fillText(total, center.x-15, center.y+5);
